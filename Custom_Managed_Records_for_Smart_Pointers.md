...
https://blog-grijjy-com.translate.goog/2020/08/12/custom-managed-records-for-smart-pointers/?_x_tr_sl=auto&_x_tr_tl=ru&_x_tr_hl=ru&_x_tr_pto=wapp

## О смарт-указках

Но сначала давайте разберёмся с терминологией. «Умные указатели» — это общее название различных технологий, упрощающих управление жизненным циклом объекта. В [предыдущей статье](https://blog-grijjy-com.translate.goog/2020/08/03/automate-restorable-operations-with-custom-managed-records/?_x_tr_sl=auto&_x_tr_tl=ru&_x_tr_hl=ru&_x_tr_pto=wapp) я показал простую `TAutoFree`CMR (Custom Managed Record), которую можно использовать для автоматического освобождения объекта, когда CMR выходит из области видимости. Это несколько похоже на `std::unique_ptr`тип в C++.
...
Кроме того, это не единственные способы реализации умных указателей. В прошлом было предпринято множество более или менее успешных попыток создания умных указателей с помощью Delphi. Например, Руди Велтхейс представил подход, использующий запись [с интерфейсом, защищающим объект](https://translate.google.com/website?sl=auto&tl=ru&hl=ru&client=webapp&u=http://rvelthuis.blogspot.com/2017/09/smart-pointers.html) . Барри Келли демонстрирует довольно оригинальный подход с использованием [анонимных методов](https://translate.google.com/website?sl=auto&tl=ru&hl=ru&client=webapp&u=http://blog.barrkel.com/2008/11/somewhat-more-efficient-smart-pointers.html) , а Джаррод Холлингворт развивает его, предлагая собственную [реализацию умного указателя](https://translate.google.com/website?sl=auto&tl=ru&hl=ru&client=webapp&u=https://adugmembers.wordpress.com/2011/12/05/smart-pointers/) .
...
Если хотите попробовать, загляните в наш репозиторий [JustAddCode](https://translate.google.com/website?sl=auto&tl=ru&hl=ru&client=webapp&u=https://github.com/grijjy/JustAddCode/tree/master/CustomManagedRecords) за примерами реализаций. Вы можете модифицировать их по своему усмотрению.